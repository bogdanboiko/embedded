...
static void adc_read_task(void* arg) {
    ...
    // For ESP32C6 ADC1 Channel 0 is usually GPIO 0. Let's use Channel 0 for
    // now.
    ...
    while (1) {
        if (s_reconfig_needed) {
            ...
            if (adc_handle) {
                ...
                if (ret != ESP_OK) {
                    ESP_LOGE(TAG, "adc_continuous_stop failed: %s",
                             esp_err_to_name(ret));
                }
            ...
            // For now, we rely on the global s_sample_rate etc being read by
            // init
            adc_channel_t channel[1] = {ADC_CHANNEL_0};
        ...
        ret = adc_continuous_read(adc_handle, result,
                                  get_optimal_buffer_size(s_sample_rate),
                                  &ret_num, 0);
        if (ret == ESP_OK) {
            // ESP_LOGI(TAG, "ret is %x, ret_num is %"PRIu32" bytes", ret,
            // ret_num);
            ...
            // The original code was: `uint16_t val = (uint16_t)data;` and sent
            // that. So we have 1/2 the size.

            if (s_ws_client_fd != -1) {
                // Allocate a small temp buffer on stack or static to avoid
                // malloc in loop ret_num is up to ADC_READ_LEN (1024). 1024 / 4
                // = 256 samples. 256 * 2 bytes = 512 bytes output. Stack safe.
                ...
                if (out_idx > 0) {
                    ...
                    esp_err_t ret_ws = httpd_ws_send_frame_async(
                        s_server, s_ws_client_fd, &ws_frame);
                    if (ret_ws != ESP_OK) {
                        ESP_LOGW(TAG, "dropped: %s", esp_err_to_name(ret_ws));

                        // Invalidate FD if it's no longer valid (client
                        // disconnected)
                        if (ret_ws == ESP_ERR_INVALID_ARG ||
                            ret_ws == ESP_FAIL) {
                            s_ws_client_fd = -1;
                        }
                    }
             ...
             * Yield check
             */
            taskYIELD(); /* Explicit yield to let WiFi stack run if needed,
                            though send_async should handle it */
        } else if (ret == ESP_ERR_TIMEOUT) {
            // We try to read `ADC_READ_LEN` until API returns timeout, which
...
static void start_test_signal(uint32_t hz) {
    ...
    ledc_channel_config_t ledc_channel = {
        ...
        .duty =
            ...
            << (ledc_timer.duty_resolution -
                1),  // 512, // 50% duty cycle (1024 / 2 for 10-bit resolution)
        .hpoint = 0};
    ...
    ledc_set_duty(ledc_channel.speed_mode, ledc_channel.channel,
                  ledc_channel.duty);  // 50% duty cycle
...
buzzer_note_t map_touch_pad_to_note(int pad_index) {
    int clampedIndex = pad_index >= BUZZER_SOUND_LIBRARY_SIZE
                           ? BUZZER_SOUND_LIBRARY_SIZE - 1
                           : pad_index;
    return buzzer_sound_library[clampedIndex];
}

static TaskHandle_t sound_play_task_handler = NULL;
static buzzer_sound_t playing_sound;

void play_sound_task(void* args) {
     buzzer_set_duty(0);
    buzzer_sound_t sound = playing_sound;
    buzzer_set_duty(4095);
    buzzer_set_pitch(sound.note.tone);
    vTaskDelay(pdMS_TO_TICKS(sound.duratuion));
    buzzer_set_duty(0);
    sound_play_task_handler = NULL;
    vTaskDelete(NULL);
}
...
#include CONFIG_BOARD_SPECIFIC_INIT
    ...
    // Wait for WiFi connection (Not strict blocking anymore, manager handles
    // it) But let's keep it to print status Note: s_wifi_event_group is local
    // to this file, we removed it in place of manager's. Proper way: expose
    // event group from manager or just dont block here. For now, let's just
    // proceed. The webserver will start and wait for connections.
    ...
    int last_touching_pad_index = -1;

    while (1) {
        bool isTouching = fruits_is_touching();
        int currently_touching_pad_index = fruits_active_index();

        if (!isTouching) {
            last_touching_pad_index = -1;
            vTaskDelay(pdMS_TO_TICKS(20));
            continue;
        }

        if (isTouching) {
            last_touching_pad_index = currently_touching_pad_index;

            buzzer_note_t note =
                map_touch_pad_to_note(currently_touching_pad_index);
            buzzer_sound_t sound = {
                .note = note, .duratuion = 500, .duty = fruits_read_pot()};

            playing_sound = sound;

            if (sound_play_task_handler != NULL) {
                vTaskDelete(sound_play_task_handler);
                sound_play_task_handler = NULL;
            }

            xTaskCreate(play_sound_task, "Play sound task", 8192, NULL, 10,
                        &sound_play_task_handler);
        }
...
static esp_err_t ws_handler(httpd_req_t* req) {
    ...
    if (ws_pkt.len) {
        ...
        if (ws_pkt.type == HTTPD_WS_TYPE_TEXT &&
            ...
            ESP_LOGI(TAG, "New WS client connected, fd=%d",
...
static esp_err_t params_handler(httpd_req_t* req) {
    ...
    if (root) {
        ...
        ESP_LOGI(TAG,
                 "Config Request: Rate=%lu, Atten=%d, Width=%d, TestHz=%u, "
                 "s_reconfig_needed=%d",
                 s_sample_rate, s_atten, s_bit_width, s_test_hz,
                 s_reconfig_needed);
